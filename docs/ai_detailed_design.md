...
| F13    | 複数客対応           | 前のお客さんが購入を終了したら、システムを終了せずに次のお客さんが連続して利用できる（1人ずつ順番に利用、同時利用は不可） |
...
| 同時利用 | 1人ずつ順番に利用し、前の取引が完了したら次の利用者が操作できる。システムは終了せず連続利用が可能。同時利用・排他制御は不要。 |
...
| セッション情報 | F13 | 1取引ごとにセッションIDを発行し、各お客さんの利用履歴を区別するための情報。1人の取引が終わるごとに新しいセッションIDを発行し、同時利用は行わない。 |
...
> 本システムの「複数客対応」とは、1人のお客さんの購入が完了した後、システムを終了せずに次のお客さんが続けて利用できることを指します。
> 1人の取引が完了するごとに新しいセッションIDを発行し、各取引の履歴を区別します。
> 複数人が同時に操作すること（同時利用）は想定していません。排他制御や同時アクセスの考慮は不要です。
> 画面上は「次のお客様どうぞ」などの案内を表示し、連続利用を促します。
